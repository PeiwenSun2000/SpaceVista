"""
SpaceVista-Bench API Evaluation Script (Video Input).

Evaluates bench_all_clean_corrected.json using VLM APIs (e.g. Qwen2.5-VL-72B via OpenRouter).
Sampled frames are synthesized into an MP4 video in memory and sent as a single
base64 video to the API, instead of multiple individual images.

Requires ffmpeg to be installed and available on PATH.

Usage:
    python evaluate_api_video.py                           # full run with defaults
    python evaluate_api_video.py --debug --debug_size 5    # quick test on 5 entries
    python evaluate_api_video.py --category Indoor          # evaluate one scale only
    python evaluate_api_video.py --resume results.jsonl     # resume interrupted run
    python evaluate_api_video.py --metrics_only results.jsonl  # recompute metrics
    python evaluate_api_video.py --fps 1                    # 1 frame per second
"""
...
def frames_to_video_bytes(frames: List[Image.Image], fps: int = 2) -> bytes:
    """Encode PIL Image frames into H.264 MP4 bytes using ffmpeg."""
    if not frames:
        return b""
    w, h = frames[0].size
...
def encode_video(frames: List[Image.Image], fps: int = 2) -> str:
    """Encode frames to MP4 and return base64 string."""
    video_bytes = frames_to_video_bytes(frames, fps)
...
def build_annotation_list(
    item: Dict[str, Any], frame_paths: List[str]
) -> List[Dict[str, Any]]:
    """Convert annotations dict into a flat list of drawing instructions."""
    ...
    if "mask" in input_type:
        ...
        for i, mask_info in enumerate(masks):
            raw_mask_path = mask_info.get("mask_path")
            object_ids = mask_info.get("object_ids")
            image_path = mask_info.get("image_path")

            # Handle nested dict format: mask_path can be a dict with
            # {image_path, mask_path, object_ids} instead of a plain string
            if isinstance(raw_mask_path, dict):
                image_path = raw_mask_path.get("image_path", image_path)
                object_ids = raw_mask_path.get("object_ids", object_ids)
                raw_mask_path = raw_mask_path.get("mask_path")

            frame_idx = _find_frame_index(image_path, frame_paths)
            if raw_mask_path and isinstance(raw_mask_path, str):
                arr = load_mask_array(raw_mask_path)
                if arr is not None:
    ...
    if dual_bbox and "bbox" in input_type:
        ...
        if src_bbox:
            out.append({
                "type": "bbox", "frame_idx": src_idx,
                "color": "red", "data": src_bbox,
            })
        for j, tb in enumerate(tgt_bboxes):
            out.append({
                "type": "bbox", "frame_idx": tgt_idx,
...
def call_api(
    ...
    """Call API with frames synthesized into a single MP4 video."""
    b64_video = encode_video(frames, fps)
    content = [
...
def process_single_entry(
    ...
    """Process one benchmark entry: load frames, render annotations, synthesize video, call API.
    ...
    # 4. Resize after annotation rendering for video encoding
    frames = [resize_image(f, args.max_image_size) for f in frames]

    prompt = build_prompt(entry)
    prediction = call_api(
        client, args.model, frames, prompt,
        fps=args.fps, max_retries=args.max_retries, max_tokens=args.max_tokens,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate SpaceVista-Bench using VLM API with video input."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir", type=str, default="./eval_results_api_video",
        help="Output directory",
    )
    parser.add_argument("--num_frames", type=int, default=16)
    parser.add_argument("--max_image_size", type=int, default=448)
    parser.add_argument("--fps", type=int, default=2,
                        help="Frames per second for synthesized video (default: 2)")
    ...
    print(f"Frames per entry: {args.num_frames}, max image size: {args.max_image_size}, fps: {args.fps}")
