...
def vsibench_aggregate_results(results):
    ...
    # quit()
    output = {}
    question_type_set = set(results['question_type'])
    # print(question_type_set)
...
def spacevista_aggregate_results(results):
    ...
    if 'task_type' in results_df.columns:
        for task_type, group_indexes in results_df.groupby('task_type').groups.items():
            ...
            if 'accuracy' in per_task_type_df.columns:
                mean_accuracy = per_task_type_df['accuracy'].dropna().mean()
                output[f"{task_type}_accuracy"] = mean_accuracy

            if 'MRA:.5:.95:.05' in per_task_type_df.columns:
                mean_mra = per_task_type_df['MRA:.5:.95:.05'].dropna().mean()
                output[f"{task_type}_MRA"] = mean_mra

    else:
        print("Warning: 'task_type' column not found in results. Cannot provide a breakdown.")

    all_accuracy_values = pd.Series(dtype=float)
    if 'accuracy' in results_df.columns:
        all_accuracy_values = pd.concat([all_accuracy_values, results_df['accuracy'].dropna()])

    if 'MRA:.5:.95:.05' in results_df.columns:
        all_accuracy_values = pd.concat([all_accuracy_values, results_df['MRA:.5:.95:.05'].dropna()])

    if not all_accuracy_values.empty:
        output['overall_accuracy'] = all_accuracy_values.mean()
        
    if not output:
        print("Error: No metrics were calculated. Please check your data and question types.")
