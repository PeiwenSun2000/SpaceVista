...
import fsspec
from datasets import DatasetDict, concatenate_datasets, interleave_datasets, Value
...
def to_json_str(e):
    ...
    if isinstance(x, str):
        ...
        try:
            json.loads(x)
            return {"_extra_info": x}
        except Exception:
            return {"_extra_info": json.dumps({"raw": x})}
    # 任意结构 -> JSON
    return {"_extra_info": json.dumps(x)}

def normalize(ds):
    # 确保列存在
    # Related to version of dataset F**K...
    if "_extra_info" not in ds.column_names:
        ds = ds.map(lambda _: {"_extra_info": "{}"}, remove_columns=[])
    else:
        ds = ds.map(to_json_str, remove_columns=[])
    # 统一类型
    ds = ds.cast_column("_extra_info", Value("string"))
    return ds
...
def merge_dataset(
    ...
    if len(all_datasets) == 1:
        all_datasets = [normalize(ds) for ds in all_datasets]
